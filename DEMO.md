...
**Steps:**
1. Upload initial CSVs to `gs://pmgraham-biglake-pipeline/inbox/{table_name}/`
...
SELECT
  ...
  u.country,
  AI.GENERATE_TEXT(
    MODEL `biglake-iceberg-datalake.llm.gemini_2_5_flash`,
    CONCAT(
...
```sql
CREATE TABLE `biglake-iceberg-datalake.gold.product_catalog`
AS SELECT
  id AS product_id,
  name,
  category,
...
FROM `biglake-iceberg-datalake.silver.products`;
```

2. Create an embedding model connection and generate embeddings:

```sql
CREATE MODEL `biglake-iceberg-datalake.gold.embedding_model`
  REMOTE WITH CONNECTION `biglake-iceberg-datalake.us-central1.biglake-iceberg`
  OPTIONS (ENDPOINT = 'text-embedding-005');

ALTER TABLE `biglake-iceberg-datalake.gold.product_catalog`
  ADD COLUMN description_embedding ARRAY<FLOAT64>;

UPDATE `biglake-iceberg-datalake.gold.product_catalog` p
SET description_embedding = (
  SELECT ml_generate_embedding_result
  FROM ML.GENERATE_EMBEDDING(
    MODEL `biglake-iceberg-datalake.gold.embedding_model`,
...
ON `biglake-iceberg-datalake.gold.product_catalog`(description_embedding)
...
SELECT
  ...
  distance
FROM VECTOR_SEARCH(
  TABLE `biglake-iceberg-datalake.gold.product_catalog`,
  'description_embedding',
  (SELECT description_embedding FROM `biglake-iceberg-datalake.gold.product_catalog` WHERE product_id = 42),
  top_k => 5,
...
```sql
CREATE TABLE `biglake-iceberg-datalake.gold.customer_profiles`
AS SELECT
  cm.user_id,
  cm.email,
  cm.country,
...
FROM `biglake-iceberg-datalake.gold.customer_metrics` cm;
```
...
- **GCS Bucket:** `pmgraham-biglake-pipeline` (inbox/ → staging/ → iceberg/)
...
- **GCP Project:** `biglake-iceberg-datalake`
